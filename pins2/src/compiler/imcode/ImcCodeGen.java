...
public class ImcCodeGen implements Visitor {
    ...
    public void visit(AbsFor acceptor) {
        ...
        int low, high, op = ImcBINOP.LTH;
        if (acceptor.lo instanceof AbsAtomConst && acceptor.hi instanceof AbsAtomConst) {
            low = Integer.parseInt(((AbsAtomConst) acceptor.lo).value);
            high = Integer.parseInt(((AbsAtomConst) acceptor.hi).value);
            if (low > high) {
                op = ImcBINOP.GTH;
            }
        }

        ImcBINOP cond = new ImcBINOP(op, (ImcExpr) imcCodeDesc.get(acceptor.count), (ImcExpr) imcCodeDesc.get(acceptor.hi));
