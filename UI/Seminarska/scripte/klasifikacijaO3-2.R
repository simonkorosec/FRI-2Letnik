...
formula <- as.formula("O3 ~ Temperatura_lokacija_max + Mesec + Vlaga_mean + PM10 + Glob_sevanje_mean + Temperatura_Krvavec_min + Pritisk_min + Padavine_mean + Temperatura_Krvavec_max + Pritisk_mean + Vlaga_min + Temperatura_Krvavec_mean + Pritisk_max + Postaja + Sunki_vetra_min + Temperatura_lokacija_mean + Hitrost_vetra_mean + Sunki_vetra_max + Padavine_sum + Vlaga_max + Letni_cas + Hitrost_vetra_min")
...
sel <- sample(1:nrow(pod), size=as.integer(nrow(pod)*0.75), replace=F)
learn <- pod[sel,]
test <- pod[-sel,]
...
modelKNN <- CoreModel(formula , learn, model="knn", kInNN = 5)
...
bag <- bagging(formula , learn, nbagg=30)
...
res <- errorest(O3~., data=pod, model = rpart, predict = mypredict, est.para=control.errorest(k = nrow(pod)))
res
