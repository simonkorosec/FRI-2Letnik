...
class Graph {
    ...
    String[] dfs() {
        ...
        array = izhodDFS.getReverseArray();
        for (int anArray : array) {
            sb.append(anArray).append(" ");
        }
    ...
    private void dfs(int n, boolean[] obiskani) {
        ...
        this.vhodDFS.add(n); // Dodaj ko pridem
        obiskani[n] = true;
        ...
        this.izhodDFS.add(n); // Dodaj ko zapustim
    }
    ...
    private void bfs(boolean[] obiskani, ResizableArray zaPregled) {
        if (zaPregled.isEmpty()) {
            return;
        }

        int i = zaPregled.dequeue();
        if (obiskani[i]) {
            if (zaPregled.isEmpty()) {
                return;
            }
        ...
        for (int j = 0; j < this.povezave[i].length; j++) {
            if (this.povezave[i][j] == 1 && !obiskani[j]) {
                zaPregled.add(j);
            }
        }
...
class ResizableArray {
    ...
    void fixArray() {
        ...
        System.arraycopy(this.array, this.front, tmp, 0, this.size);
        this.array = tmp;
        this.front = 0;
