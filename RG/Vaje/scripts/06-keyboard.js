...
function initTextures() {
  ...
  // create set of 3 textures for different filters
  for (var i=0; i < 3; i++) {
    var texture = gl.createTexture();
    texture.image = cubeImage;
    cubeTextureSet.push(texture);
  }
  cubeImage.onload = function () {
    handleTextureLoaded(cubeTextureSet)
  }; // async loading
  cubeImage.src = "./assets/crate.gif";
}

function handleTextureLoaded(textures) {
  gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);

  // First texture uses Neares neighbor approximation
  gl.bindTexture(gl.TEXTURE_2D, textures[0]);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, textures[0].image);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);

  // Second texture uses Linear interpolation approximation
